...
class GetFeed(object):
    ...
    def _reord(self, d):
        ...
        d['name'] = d['title'] \
            .map(lambda x: re.match(self.feed.regex, x, re.IGNORECASE) \
                 .group(1).strip() if re.match(self.feed.regex,
                                               x, re.IGNORECASE) else numpy.nan)
        d.dropna(inplace=True)
        if feed.has_episodes:
            d['episode'] = df['title'] \
                .map(lambda x: int(re.match(self.feed.regex, x,
                                            re.IGNORECASE).group(2)))
        if feed.has_seasons:
            d['season'] = df['title'] \
                .map(lambda x: int(re.match(self.feed.regex, x,
                                            re.IGNORECASE).group(3)))
        if feed.has_episodes and feed.has_seasons:
