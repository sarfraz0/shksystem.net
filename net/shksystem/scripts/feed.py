...
from net.shksystem.common.utils import regexify
...
class ReleaseFrame(object):
    ...
    def get(self, kickass_url, strike_url, regex, has_episodes=False,
            has_seasons=False):
        ret = None
        retry = False
        try:
            ret = self._get_from_kickass(kickass_url, regex, has_episodes,
                                         has_seasons)
        except:
        ...
        if retry:
            try:
                ret = self.get_from_strike(strike_url, regex, has_episodes,
                                           has_seasons)
            except:
                logger.exception('Cannot get feed from Strike API.')
        return ret
